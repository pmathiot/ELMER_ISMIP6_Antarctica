  ...
  CONTAINS
    ...
    SUBROUTINE NetCDFInit(Params, NetDim, DimSize, CoordVarNDims, TimeSize, x0, dx, t0, dt, UniformCoords )
      ...
      IF ( NetCDFstatus /= NF90_NOERR ) THEN
          PRINT *,'NetCDF file could not be opened: '//TRIM(FileName)
...
CONTAINS
  ...
  !----------------------------------------------------
    ...
    REAL(KIND=dp) :: Time, Coeff, IndexRead, Offset
    ...
    ! New for our problem
    IndexRead = GetCReal( Params, "Index Read", Found )
    IF( Found ) THEN
        !PRINT *,'Indexreadfinal', IndexRead
        TimeIndex = IndexRead
        Offset = GetCReal( Params, "Time Offset", Found )
        IF( Found ) THEN 
           !PRINT *,'Offsetfinal', Offset
           TimeIndex = IndexRead + Offset
        END IF
        !PRINT *,'TImeIndexfinal', TimeIndex
        RETURN
    END IF
