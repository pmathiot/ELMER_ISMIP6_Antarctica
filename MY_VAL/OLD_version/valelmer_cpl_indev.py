...
def compute_nc_data(runid_lst, cdir):
    ...
    for runid in runid_lst:
        ...
        cfile_states = f'{cdir}/{runid}-S/ist/????/{runid}_antarctica_ismip6_states_????1231.[0-9]*[0-9].nc'
        cfile_fluxes = f'{cdir}/{runid}-S/ist/????/{runid}_antarctica_ismip6_fluxes_????1231.[0-9]*[0-9].nc'
        ...
        for cbasin, mask in mask_dict.items():
            ...
            for ikey, ckey in enumerate(PLOT_KEYS):
                ...
                da_int = (ds[cvar] * ds['cell_area']).sum(dim='nantarctica_face') * PLOT_SF[ikey]
    ...
    for cbasin in basin_list:
        ...
        for i, (ckey, unit) in enumerate(zip(PLOT_KEYS, PLOT_UNITS)):
            ...
            for ds, (name, style, color) in zip(data_states if PLOT_NCFIL[i] == 'states' else data_fluxes, styles):
                ...
                df.index = df.index.strftime('%Y').astype(int)  # Convert datetime index to string format (years only)
                if PLOT_NCFIL[i] == 'states':
                    df.index = (df.index - 1)#.astype(str)  # Convert to int, subtract 1, then back to string
